...
# /root/miniconda3/envs/moonraker_py/bin/python /root/moonraker/moonraker/moonraker.py -l /root/klipper_logs/moonraker.log -c /moonraker.conf
warn_msg(){
...
moonraker_setup(){
  ...
  [ ! -d "$MOONRAKER_DIR" ] && cd "${HOME}" && git clone $MOONRAKER_REPO

  ok_msg "Download complete!"

  ### step 2: install moonraker dependencies and create python virtualenv
  status_msg "Installing dependencies ..."
  [ ! -d "$MOONRAKER_DIR" ] && install_moonraker_packages
...
create_moonraker_virtualenv(){
  ...
  if /root/miniconda3/bin/conda env list | grep ".*moonraker_py.*" >/dev/null 2>&1; then
    status_msg "moonraker_py environment exist!"
  else
    status_msg "create moonraker_py environment"
    /root/miniconda3/bin/conda create -n moonraker_py python=3.8 -y
  fi
  # Install/update dependencies
  conda activate moonraker_py

  ### Install/update dependencies
  pip install -r "$MOONRAKER_DIR"/scripts/moonraker-requirements.txt 
...
create_moonraker_service(){
  ...
  MR_SERV_SRC="./resources/moonraker.service"
  MR_SERV_TARGET="$SYSTEMDDIR/moonraker.service"

  write_mr_service(){
    if [ ! -f $MR_SERV_TARGET ]; then
      status_msg "Creating Moonraker Service $i ..."
        cp "$MR_SERV_SRC" $MR_SERV_TARGET
        sed -i "s|%INST%|$i|" $MR_SERV_TARGET
        sed -i "s|%USER%|${USER}|" $MR_SERV_TARGET
        sed -i "s|%MR_ENV%|$MR_ENV|" $MR_SERV_TARGET
        sed -i "s|%MR_DIR%|$MR_DIR|" $MR_SERV_TARGET
        sed -i "s|%MR_LOG%|$MR_LOG|" $MR_SERV_TARGET
        sed -i "s|%MR_CONF%|$MR_CONF|" $MR_SERV_TARGET
    fi
  }
...
create_moonraker_conf(){
  ...
  MR_CONF_SRC="./resources/moonraker.conf"
  mr_ip_list=()
...
moonraker_polkit(){
  ...
  for service in $MOONRAKER_SERVICES; do
    ...
    if [ "$HAS_SUPP" -eq 0 ]; then
      ...
      sed -i "/^Type=simple$/a SupplementaryGroups=moonraker-admin" "$service" \
